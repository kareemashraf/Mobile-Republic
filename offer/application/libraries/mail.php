			...
			if (!$handle) {
				...
				//if (!empty(self::$username)  && !empty(self::$password)) {
					...
					if (substr($reply, 0, 3) == 334) {
						...
						fputs($handle, 'AUTH PLAIN' . self::$crlf);
						...
						if (substr($reply, 0, 3) != 334) {
							trigger_error('Error: AUTH LOGIN or PLAIN not accepted from server!');
							exit();						
						} else {
							fputs($handle, base64_encode("\000".self::$username."\000".self::$password) . self::$crlf);
							...
							if (substr($reply, 0, 3) != 235) {
								trigger_error('Error: Username/Password not accepted from server!');
								exit();								
							}
						}
